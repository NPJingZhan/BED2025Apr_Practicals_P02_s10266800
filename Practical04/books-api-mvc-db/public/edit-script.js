...
editBookForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.json();
      throw new Error(errorBody.error || "Failed to update book");
    }

    messageDiv.textContent = "Book updated successfully!";
    messageDiv.style.color = "green";

    // Optionally, redirect back to the index page on successful update
    setTimeout(() => {
      window.location.href = "index.html";
    }, 1000);

  } catch (error) {
    // Provide feedback to the user using the messageDiv (error)
